...
that we discussed on Page~\ref{page:test for conservative vector field}.)
...
\int_{\partial D} P\,dx +Q\,dy = \iint_{D} Q_x-P_y \,dA
...
Theorem (\ref{thm:greens theorem}) except that $P$ and $Q$ have
traded places and $Q$ has acquired a negative sign. Then applying
Green's Theorem we get 
$$
\int_{\partial D} - Q\,dx+P\,dy=\iint_{D} P_x+Q_y\,dA=
\iint_{D} \nabla\cdot{\bf F}\,dA.$$
...
\begin{ex}
If $\nabla \cdot F=0$, $F$ is said to be \dfont{incompressible}.  Show that any vector field
